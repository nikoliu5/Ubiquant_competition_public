...
class BotsDemoClass(BotsClass):
    ...
    def process_all_stocks(self):
        with ThreadPoolExecutor(max_workers=8) as executor:
            futures = [
                executor.submit(self.fetch_stock_data, stock) for stock in range(29)
            ]
            for future in futures:
                try:
                    ...
                    # Handle exceptions
                    print(f"Error in fetch_trade_data: {e}")

    def fetch_stock_data(self, stockID):
...
def main():
    bot = BotsDemoClass("UBIQ_TEAMXXX", "XXXXXXXXX")
    bot.login()
    bot.init()
    SimTimeLen = 14400
    endWaitTime = 300
